...
#ifndef LOCALIZER_MISC_H
#define LOCALIZER_MISC_H
...
// Create the directory if it does not exist.
void ReCreateDirIfNotExists(const std::string& path);
...
// Print first-order heading with over- and underscores to `LOGD(STINFO)`.
void PrintHeading1(const std::string& heading);

// Print second-order heading with underscores to `LOGD(STINFO)`.
